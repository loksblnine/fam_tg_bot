...
const start = async () => {
  ...
  bot.onText(/^\W*(?:\w+\b\W*){50,}$/, async (msg) => {
    const {chat} = msg;
    const chatId = chat.id;
    bot.sendMessage(chatId, `[Дружище](tg://user?id=${msg.user.id}), судя по ответам тут и на подслушано, должно было стать понятно, что в твоем вопросе не хватает данных для внятного ответа. 1ый корпус - здание. Есть номер ректора, его приемной, пресс-службы, проходной, профкома, учебного отдела, приемной комиссии, разных кафедр которые там обитают. В теории, можно найти номер телефона уборщицы/уборщика. Уточни кто именно тебя интересует или чего ты хочешь добиться, и тогда мы попробуем помочь)`)
  })
